...
function PhaseZeroPage() {
  ...
  return <PhaseZeroView eventDetail={eventDetail} isParticipantOnly={isParticipantOnly} />;
}

function PhaseZeroView({ 
  eventDetail, 
  isParticipantOnly 
}: { 
  ...
  useEffect(() => {
    ...
    if (current !== String(phaseZero.id)) {
      const next = new URLSearchParams(searchParams);
      next.set('phase', String(phaseZero.id));
      setSearchParams(next, { replace: true });
    }
  }, [phaseZero, searchParams, setSearchParams]);

  const activePhaseIdParam = searchParams.get('phase');
  const activePhaseId = activePhaseIdParam ? Number(activePhaseIdParam) : phaseZero?.id ?? null;
  const activePhase = phases.find(phase => phase.id === activePhaseId) ?? phaseZero ?? null;
